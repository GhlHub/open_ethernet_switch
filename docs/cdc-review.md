...
| 14 | SFP PCS gearbox (125 MHz <-> 62.5 MHz) | "Safely Timed" | Related-clock crossing from one MMCM. The SFP investigation found that meeting timing did not preserve byte pairing in the original gearbox. It now commits full TX words and retains each RX high byte while reading its low byte; four-phase regression passes. Static timing and hardware packet validation remain required. |
