...
Date: 2026-09-17. These checks were run against the existing source before its
initial commit. RTL and testbench contents were preserved during the inventory.
...
Every target was rebuilt using `make -B -C sim TARGET`. All eleven returned
...
| `sim-sfp-pcs` | [`tb_sfp_1000base_x_pcs.sv`](../tb/tb_sfp_1000base_x_pcs.sv) | Symbol-level loopback, sync acquire/loss/recovery, frame/error propagation | PASS |
...
controller. Forwarding masks come from testbench stimulus; no tested path
currently combines header parsing, learning, lookup, and packet forwarding.
The SFP tests loop back decoded parallel symbols; no serial link or optical
module participates. GEM tests use behavioral FIFO stimulus, not the hard GEM.
...
Run the ten distinct subsystem lint targets with:

```bash
make -k -C sim lint-mac lint-bufmgr lint-ingress lint-egress \
    lint-ps-eth lint-async-fifo lint-pl-gmii lint-sfp-pcs \
    lint-sfp-port lint-cpu-port
```

| Target(s) | Result |
| --- | --- |
| `lint-mac`, `lint-bufmgr`, `lint-ingress`, `lint-egress`, `lint-ps-eth`, `lint-async-fifo`, `lint-sfp-pcs`, `lint-cpu-port` | PASS, no reported warnings |
| `lint-pl-gmii` | FAIL: Verilator exits due to 31 warnings |
| `lint-sfp-port` | FAIL: Verilator exits due to 36 warnings |

Both failing targets report 24 `WIDTHEXPAND` and five `WIDTHTRUNC` warnings
in `open_eth_mac_1g_switch.sv`, plus a `SYMRSVDWORD` warning for `interrupt`.
The remaining warnings are `TIMESCALEMOD`: one for the PL target and six for
the SFP target. Warnings were not suppressed to obtain a clean result.
