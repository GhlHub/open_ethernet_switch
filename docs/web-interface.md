...
- `/configuration`: enable/disable GEM0 (right upper), GEM1 (right lower),
  ...
  Reload status fetches the latest state without applying edits.
...
restart. The CPU virtual port is not administratively configurable. Disabling
...
sensor validity and collector availability are visible. DDR/debug cycles are
...
| `POST /api/ports` | Form body `mask=0` through `mask=31`; responds with the requested administrative and current physical/forwarding state |
...
Port changes do not alter IP configuration or restart the network stack.
