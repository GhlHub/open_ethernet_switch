...
frequency register drives new HTTP/SNMP frequency reporting. Current counter
widths still cover half a second at the higher rate. The 125 MHz build passes
routed timing (fabric +1.655 ns setup; overall +0.018 ns setup / +0.010 ns hold)
and is deployed with matching R5 firmware. DHCP, HTTP/SNMP frequency reporting,
and 1,000 full-size pings each to CPU and forwarded miner passed. See
[board validation](verification.md#2026-09-26-125-mhz-fabric-board-deployment).

Future work: **widen the SFP packet interface to 128 bits** for a high-speed
1/2.5/5/10 Gb/s trunk. Keep byte enables and frame boundaries, adapt buffering
and parsing, revisit byte-counter widths and qualify DDR throughput. The current
SFP remains 1 Gb/s; its 16-bit fabric interface is limited to 2 Gb/s raw at
125 MHz. Faster MAC/PCS and transceiver support remain separate work.
...
validation evidence and remaining physical-shell migration. Production
`system.bd` now instantiates all seven catalog cells directly; that newer
assembly passed generated-datapath simulation and bitstream generation
(WNS +0.018 ns, hold +0.011 ns) and passed JTAG deployment with matching
R5 firmware, settled GEM1/PL0 connectivity, SNMP and HTTP checks.
The preceding native all-counter image was routed and loaded over JTAG with matching
...
| --- | --- | --- |
| Digital switch assembly | [`switch_top.sv`](../rtl/switch_top.sv) | Joins two GEM bridges, two PL MAC ports, one SFP MAC/PCS port, physical ingress/egress, the CPU port, and MAC forwarding. The extracted `switch_fabric` generates the aging tick (default 4 Hz at 125 MHz). GEM0-to-CPU smoke test passes. Production uses the catalog instances in `system.bd`; this native assembly remains the simulation reference. Separate GEM RX/TX clocks replace the former single-clock interface. Synchronizes per-port `fwd_en_i`/`learn_en_i` into the fabric clock; CPU TX destination metadata now travels in the frame header and is held through enqueue; see control-protocol hooks in [architecture](architecture.md#control-protocol-hooks-stplacplldp-no-protocol-logic). |
...
| PS GEM port | [`ps_gem_axis_bridge.sv`](../rtl/ps_eth/ps_gem_axis_bridge.sv), [`gem_rx_w_to_axis.sv`](../rtl/ps_eth/gem_rx_w_to_axis.sv), [`axis_to_gem_tx_r.sv`](../rtl/ps_eth/axis_to_gem_tx_r.sv) | One reusable full-duplex GEM FIFO/AXI-S bridge, instantiated twice as `gem_port` catalog cells in production and retained in `switch_top` for simulation. Separate RX/TX clocks, RX overflow/flush bad-frame termination and TX underflow/drain/flush recovery exist. Recovery tests pass; throughput, simultaneous-event corners and real GEM timing remain open. |
| PL MAC port | [`pl_gmii_mac_top.sv`](../rtl/pl_gmii/pl_gmii_mac_top.sv), [`open_eth_mac_1g_switch.sv`](../rtl/pl_gmii/open_eth_mac_1g_switch.sv) | 1G full-duplex GMII MAC, switch-oriented receive acceptance, AXI-Lite register access, and wrapper. Instantiated twice as `pl_port` catalog cells in production and retained in `switch_top` for simulation. The board wrapper joins both GMII interfaces to RGMII adapters. The imported core's resets are reclocked into each clock domain it uses (`tb_mac_reset_reclock.sv`). Its data read pointers are published to the other clock domain one word per clock (a CDC fix; `tb_sfp_port_top.sv` monitors it). |
...
| PL RGMII adapter | [`rgmii_gmii_adapter.sv`](../rtl/pl_gmii/rgmii_gmii_adapter.sv), [`rgmii_gmii_sim_model.sv`](../rtl/pl_gmii/rgmii_gmii_sim_model.sv) | Primitive-based DDR TX/RX conversion with optional RX clock delay and a 2048-entry FIFO36E2 elastic RX buffer. The separate behavioral model passes nibble/control loopback tests but omits the real FIFO, delay calibration, and I/O timing. The hardware module remains in `kr260_pl_top`, connected to the BD MAC cells; optional FPGA RX clock delay defaults off; board overrides set data/control delays to PL0 700 ps and PL1 750 ps (adapter default 500 ps). IDELAYCTRL readiness gates receive reset with a 64-cycle hold; the primitive startup/reset test passes. Hardware gaps are listed below. |
...
| --- | --- |
| SFP negotiation configuration | Generic timer defaults remain 8 cycles; the board overrides restart to 10 ms and acknowledge/idle timers to 10 ms at 125 MHz through the production SFP catalog-cell parameters. There is no negotiation-disable/fixed-link control. |
...
| GEM FIFO throughput (fixed; needs hardware confirmation) | The 16-bit CDC paths have a raw 2 Gb/s fabric-side capacity at 125 MHz. Tests pass a 1518-byte frame in each direction, including one RX stall in 16 cycles and modeled TX gaps. The 128-word FIFO bounds backpressure. Bench clocks are nominal 125/125 MHz with RX/TX tied together; independent drift and sustained all-port load are untested. |
