...
//      ~1.667ns (300MHz), clk_o period is 16ns (62.5MHz), measured
...
module tb_pl_eth_clk_gen_sim_model;
  ...
  initial begin
    ...
    begin
      ...
      clk_period = t1 - t0;
      if (clk_period < 15.999 || clk_period > 16.001) begin
        $display("FAIL: testB clk_o period = %0fns, expected 16ns (62.5MHz)", clk_period);
        ok = 1'b0;
      end

      if (ok) $display("PASS: testB gtx_clk_o=125MHz, idelay_refclk_o=300MHz, clk_o=62.5MHz (measured from edge timestamps)");
      else errors++;
    end
