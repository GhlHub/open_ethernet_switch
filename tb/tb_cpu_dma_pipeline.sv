`timescale 1ns/1ps
module tb_cpu_dma_pipeline;
  import buf_mgr_pkg::*;
  import axi_dma_pkg::*;
  logic clk=0, rst_n=0;
  always #4 clk=~clk;
  logic frame_ready_i='0, frame_gnt_o, frame_rd_en_o, frame_dma_done_o;
  logic [BUF_ID_W-1:0] frame_bufid_i='0;
  logic [LENGTH_W-1:0] frame_length_i='0;
  logic [AXI_DATA_W-1:0] frame_rd_data_i='0;
  ...
  always @(posedge clk) if (rst_n && frame_rd_en_o)
    frame_rd_data_i <= payload(port,int'(frame_rd_addr_o));
  ...
  always @(posedge clk) begin
    ...
    if (rst_n && active) begin
      ...
      if (|frame_gnt_o) begin
        if (frame_gnt_o !== 1'b1 || grants!=0) $fatal(1,"wrong/repeated grant");
        grants=grants+1;
      end
      if (|frame_rd_en_o) begin
        if (frame_rd_en_o !== 1'b1 || int'(frame_rd_addr_o)!=reads || reads>=beats)
      ...
      if (m_axi_wvalid && m_axi_wready) begin
        ...
        if (consecutive && writes>0 && cycle!=previous_write+1)
      ...
      if (|frame_dma_done_o) begin
        if (frame_dma_done_o !== 1'b1 || !responded || reads!=beats || grants!=1)
  ...
  task automatic start_frame(input integer p,input integer n,input bit fast);
    ...
    frame_length_i=LENGTH_W'(n);
    frame_bufid_i=BUF_ID_W'(p+7);
    frame_ready_i=1;active=1;
  endtask
  ...
  task automatic run_frame(input integer p,input integer n,input integer mode);
    ...
    while (!finished) begin
      ...
      if (mode==3 && ticks==35 && reads!=2) $fatal(1,"read-ahead did not stop at two reserved words");
    end
    if (mode==0 && n==1500)
      $display("PERF CPU length=1500 beats=%0d AW-to-last=%0d cycles data-span=%0d cycles",beats,last_cycle-aw_cycle,last_cycle-first_cycle+1);
  ...
  initial begin
    ...
    reset_engine();
    run_frame(0,1500,0);
    // Exhaustive length/partial strobe coverage, with varying buffer IDs and payload patterns.
    ...
    $display("PASS: %0d CPU DMA cases; all lengths, backpressure stability, delayed AW/B, reset and throughput",tests);
    $finish;
  end
